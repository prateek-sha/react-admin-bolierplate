...
import { Table, Button, Space, Modal, Input, InputNumber, Form, DatePicker } from 'antd'
import { getAddress, getUser, sendMailByUser, setOrder, setQuote } from '../../elements/api/other';
...
export const QuotePage = () => {
	...
	useEffect(() => {
		if (allDataReducer.quote) {
			let res = allDataReducer.quote;
			setData(res)
		}
	}, [allDataReducer.quote])

	const [priceSection, setPriceSection] = useState("ruppe")
	const sendMail = async (productData, userData) => {
								...
								<td
								  ...
								  <div class="v-text-align v-line-height"
									...
									<p style="font-size: 14px; line-height: 140%;"><span
										style="font-size: 12px; line-height: 16.8px;">${productName} </span></p>
								  </div>
	  ...
	  <body class="clean-body"
		...
		let res = await sendMailByUser({ message: mailContent, userMail });
		responseHelper(res, () => { })

	}

	const handleOrder = async (mail) => {
		const formData = form.getFieldsValue();
		let erros = form.getFieldsError()
		for(let i=0; i< erros.length;i++){
			if(erros[i].errors.length){
				toast.error(`Please fill ${erros[i].name}`)
				return;
			}
		}

		let { productID, quantity } = data.filter(dd => dd.id === nodal)[0];
		let userEmail = data.filter(dd => dd.id === nodal)[0].email;
		setMail(userEmail)
		let userData = await getUser();
		let userID
		let tempproductData = productData.filter(data => data.id === productID);
		tempproductData = tempproductData.map(Data => {
			return {
				...Data,
		...
		const handleMail = (orderID) => sendMail(tempproductData, { ...formData, email: userEmail, ...addressData, quantity, priceSection: formData.priceSection, orderID, price: formData.price });

		if (userData.status === "1") {
			if (userData.data.filter(data => data.email === userEmail).length) {
				userID = userData.data.filter(data => data.email === userEmail)[0]['id']
				const handleOk = async (accpet) => {
					let res = await setOrder({
						...
						orderStatus: "active",
						quantity,
						priceSection: formData.priceSection
					});
					let ress = await getAddress({ parentID: userID })
					if (ress.status === "1") {
				...
				toast.error("Opps no user found, first make a account")
			}
		}
	}
	...
	const columns = [
		...
		},
		{
			title: 'Action',
			render: (data) => <Space>
				{<Select defaultValue={data.actionValue} onChange={(value) => { setActionValue(value); setIsClicked(data.id) }} >
					<Select.Option value={"0"}>Waiting</Select.Option>
					<Select.Option value={"1"}>In-Progress</Select.Option>
					<Select.Option value={"2"}>Accpet</Select.Option>
				</Select>}
				<Button type="primary" onClick={() => handleUpdate(data.id)} disabled={!(isClicked !== null && isClicked === data.id)} >Save</Button>
				<Button type="primary" onClick={() => setNodal(data.id)} >Order</Button>
			</Space>
		}
	];

	const handleUpdate = async (id) => {
		let res = await setQuote({ id, actionValue, action: "UPDATE" })
			...
			>
				<>
					<Form form={form} onFinish={() => { }} >
						...
						<Form.Item name="price" label="Price" rules={[{ required: true, message: 'Missing' }]}>
							<InputNumber />
						</Form.Item>
						...
						<Form.Item name="shipping" label="Shipping " rules={[{ required: true, message: 'Missing' }]} >
							...
							<Input />
						</Form.Item><Form.Item name="tax" label="Tax" >
							<InputNumber />
						</Form.Item>
						<Form.Item name="priceSection" label="Price " >
							<Select>
						...
						<br />
						<br />
						<Button onClick={() => handleOrder(0)} >Send Request</Button>
					</Form>
				</>
			</Modal>
